...
class AudioPreprocessor:
    ...
    def preprocess_to_memory(input_path: str, smart_silence: bool) -> np.ndarray:
        """Preprocesses audio to a NumPy float32 array at 16kHz mono."""
        ...
        # Using a single FFmpeg call to pipe raw float32 PCM data to stdout
        result = subprocess.run(cmd, capture_output=True, check=True)
        return np.frombuffer(result.stdout, np.float32).copy()

    @staticmethod
    def chunk_audio(audio: np.ndarray, chunk_seconds: int) -> List[np.ndarray]:
        """Splits NumPy audio array into chunks of specified duration."""
        sample_rate = 16000
...
class WhisperEngine:
    ...
    def transcribe_chunks(
        ...
        """Transcribes a list of NumPy audio chunks and merges the results."""
        all_segments = []
        full_text = []
...
class TranscriberGUI:
    ...
    def _run_transcription(self) -> None:
        ...
        try:
            ...
            # In-memory preprocessing eliminates disk I/O bottlenecks
            ...
            # Memory-efficient slicing instead of temporary file creation
            chunks = AudioPreprocessor.chunk_audio(audio_data, chunk_seconds)
