...
class AudioPreprocessor:
    ...
    def preprocess_to_memory(input_path: str, smart_silence: bool) -> np.ndarray:
        """Preprocesses audio to a 16kHz mono float32 NumPy array in memory."""
        silence_filter = ["-af", "silenceremove=stop_periods=-1:stop_duration=0.6:stop_threshold=-45dB"] \
...
class WhisperEngine:
    ...
    def detect_language(self, audio: np.ndarray) -> Tuple[str, float]:
        """Detects the spoken language from the preprocessed audio array."""
    ...
    def transcribe_chunks(
        ...
        """Transcribes the audio in-memory by slicing into chunks."""
        all_segments = []
        full_text = []
        sample_rate = 16000
