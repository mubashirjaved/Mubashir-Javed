...
class AudioPreprocessor:
    @staticmethod
    def load_audio(input_path: str, smart_silence: bool) -> np.ndarray:
        """
        Loads audio from file, applies silence removal if requested,
        and returns a float32 NumPy array at 16kHz mono.
        """
        silence_filter = (
        ...
        # We use s16le PCM to pipe data from FFmpeg to NumPy
        cmd = [
        ...
        # Performance: Piping directly to memory avoids intermediate WAV files and disk I/O.
...
class WhisperEngine:
    ...
    def detect_language(self, audio: np.ndarray) -> Tuple[str, float]:
        # whisper.pad_or_trim expects up to 30s of audio
    ...
    def transcribe_chunks(
        ...
        num_chunks = math.ceil(len(audio) / chunk_samples) if len(audio) > 0 else 1
        ...
        for idx in range(num_chunks):
            ...
            # Performance: Passing NumPy array directly to model avoids re-loading from disk.
...
class TranscriberGUI:
    ...
    def _run_transcription(self) -> None:
        ...
        try:
            ...
            self._set_progress(0.05, "Loading/Preprocessing audio")
            # Performance: load_audio now returns NumPy array directly.
            audio = AudioPreprocessor.load_audio(input_file, self.silence_var.get())
