...
class AudioPreprocessor:
    ...
    def preprocess_to_memory(input_path: str, smart_silence: bool) -> np.ndarray:
        """Decodes audio to a mono 16kHz float32 NumPy array using FFmpeg pipe."""
        ...
        # Use stdout=PIPE to capture the raw PCM data
        result = subprocess.run(cmd, check=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        # numpy.frombuffer directly creates an array from the bytes
...
class WhisperEngine:
    ...
    def detect_language(self, audio: np.ndarray) -> Tuple[str, float]:
        """Detects language from a pre-loaded audio NumPy array."""
