...
class AudioPreprocessor:
    ...
    def preprocess_to_memory(input_path: str, smart_silence: bool) -> np.ndarray:
        """
        ⚡ Bolt: Preprocesses audio to an in-memory NumPy array.
        Uses FFmpeg pipes to eliminate intermediate disk I/O.
        """
        ...
        # Using subprocess.run with stdout=PIPE to read the raw PCM data
        result = subprocess.run(cmd, capture_output=True, check=True)
        # Convert raw PCM S16LE to float32 NumPy array normalized to [-1, 1]
        audio = np.frombuffer(result.stdout, dtype=np.int16).flatten().astype(np.float32) / 32768.0
        return audio
...
class WhisperEngine:
    ...
    def detect_language(self, audio: np.ndarray) -> Tuple[str, float]:
        """
        ⚡ Bolt: Detects language using in-memory NumPy array.
        """
    ...
    def transcribe_chunks(
        ...
        """
        ⚡ Bolt: Transcribes audio using in-memory NumPy chunking.
        Replaces file-based chunking with efficient array slicing.
        """
        all_segments = []
        full_text = []
...
class TranscriberGUI:
    ...
    def _run_transcription(self) -> None:
        ...
        try:
            ...
            # ⚡ Bolt: Full in-memory processing pipeline
