...
class AudioPreprocessor:
    ...
    def preprocess_to_memory(input_path: str, smart_silence: bool) -> np.ndarray:
        """Runs ffmpeg to convert input to 16kHz mono float32 PCM in memory."""
        silence_filter = (
        ...
        # Use subprocess.run to capture the raw PCM stream from stdout
...
class TranscriberGUI:
    ...
    def _run_transcription(self) -> None:
        ...
        try:
            ...
            chunks = [
                audio_array[i : i + samples_per_chunk]
                for i in range(0, len(audio_array), samples_per_chunk)
            ]
            self._log(f"Prepared {len(chunks)} chunk(s) in memory")
