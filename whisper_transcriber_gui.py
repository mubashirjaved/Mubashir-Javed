...
class AudioPreprocessor:
    """Handles audio loading and preprocessing using FFmpeg and NumPy."""

    @staticmethod
    def preprocess_to_memory(input_path: str, smart_silence: bool) -> np.ndarray:
        """Pipes FFmpeg output to a NumPy array for in-memory processing."""
        ...
        # Note: We use .copy() to ensure the array is writable, as Whisper/Torch might require it.
...
class WhisperEngine:
    ...
    def detect_language(self, audio: Any) -> Tuple[str, float]:
        """Detects language from audio (path or NumPy array)."""
    ...
    def transcribe_chunks(
        ...
        """Transcribes audio by slicing the NumPy array into chunks in-memory."""
        all_segments = []
        full_text = []
