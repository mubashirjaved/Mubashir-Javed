...
class AudioPreprocessor:
    ...
    def preprocess_to_memory(input_path: str, smart_silence: bool) -> np.ndarray:
        """
        Loads an audio file and returns a float32 numpy array (16kHz mono, normalized).
        Uses ffmpeg to pipe raw PCM data directly to stdout.
        """
        silence_filter = [
            "-af",
...
class WhisperEngine:
    ...
    def detect_language(self, audio_data: np.ndarray) -> Tuple[str, float]:
        """Detect language from pre-loaded numpy array."""
    ...
    def transcribe_chunks(
        ...
        """
        Transcribes the full audio array by slicing it into chunks in memory.
        """
        all_segments = []
        full_text = []
