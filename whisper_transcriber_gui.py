...
class AudioPreprocessor:
    ...
    def preprocess_to_memory(input_path: str, smart_silence: bool) -> np.ndarray:
        """Runs ffmpeg to convert input to 16kHz mono float32 PCM and returns numpy array."""
        silence_filter = (
        ...
        # np.frombuffer returns a read-only view; we copy it to ensure a writable array for torch.
...
class WhisperEngine:
    ...
    def detect_language(self, audio: np.ndarray) -> Tuple[str, float]:
        """Detects spoken language from an in-memory numpy audio array."""
    ...
    def transcribe_chunks(
        ...
        """Transcribes a list of in-memory audio chunks and merges results."""
        all_segments = []
        full_text = []
