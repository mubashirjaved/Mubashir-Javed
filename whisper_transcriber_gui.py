...
class AudioPreprocessor:
    ...
    def preprocess_to_memory(input_path: str, smart_silence: bool) -> np.ndarray:
        """
        Uses FFmpeg to decode audio, resample to 16kHz mono, and return as a numpy float32 array.
        Eliminates disk I/O for intermediate preprocessing steps.
        """
        silence_filter = (
        ...
        # Use stdout=PIPE to capture the raw PCM data and stderr=PIPE to avoid console pollution
        result = subprocess.run(cmd, check=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        # Convert raw float32 PCM data to a numpy array and copy to ensure it's writable for PyTorch
...
class WhisperEngine:
    ...
    def transcribe_chunks(
        ...
        total_chunks = math.ceil(len(audio) / samples_per_chunk) if len(audio) > 0 else 1
