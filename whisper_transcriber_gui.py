"""
Whisper Audio Transcriber GUI
Optimized for performance with in-memory audio processing.
"""

# pylint: disable=import-error, no-member, attribute-defined-outside-init
# pylint: disable=line-too-long, missing-docstring, broad-exception-caught
# pylint: disable=invalid-name, too-few-public-methods, too-many-arguments
# pylint: disable=too-many-positional-arguments, too-many-locals, too-many-instance-attributes
...
class AudioPreprocessor:
    @staticmethod
    def preprocess_to_numpy(input_path: str, smart_silence: bool) -> np.ndarray:
        """Use FFmpeg to pipe raw float32 PCM data directly to a NumPy array."""
        ...
        # Performance: In-memory pipeline avoids disk I/O and temporary WAV files.
...
class WhisperEngine:
    ...
    def transcribe_chunks(
        ...
        """Transcribe audio by slicing the in-memory array into chunks."""
        all_segments = []
        full_text = []

        # 16000 Hz is Whisper's expected sample rate
        chunk_samples = chunk_seconds * 16000
        num_chunks = math.ceil(len(audio) / chunk_samples)
...
class TranscriberGUI:
    ...
    def _run_transcription(self) -> None:
        ...
        try:
            ...
            audio_data = AudioPreprocessor.preprocess_to_numpy(input_file, self.silence_var.get())
