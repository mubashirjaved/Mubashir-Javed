...
# pylint: disable=line-too-long,missing-docstring,broad-exception-caught,invalid-name,too-few-public-methods,too-many-arguments,too-many-positional-arguments,too-many-locals,too-many-instance-attributes
import math
import os
import queue
import re
...
class HardwareProfile:
    """Data class for hardware configuration."""
    device: str
    fp16: bool
    torch_threads: int
...
class HardwareOptimizer:
    """Detects and recommends hardware settings."""
    @staticmethod
...
class AudioPreprocessor:
    """Handles audio loading and preprocessing using FFmpeg."""
    @staticmethod
    ...
    @staticmethod
    def load_audio_to_numpy(input_path: str, smart_silence: bool) -> np.ndarray:
        """Loads and preprocesses audio into a NumPy array using FFmpeg pipes."""
        silence_filter = [
        ...
        # Performance: decodes directly to memory to avoid disk I/O
...
class TextPostProcessor:
    """Cleans up and post-processes transcribed text."""
    @staticmethod
...
class WhisperEngine:
    """Wraps OpenAI Whisper for hardware-accelerated transcription."""
    ...
    def detect_language(self, audio: np.ndarray) -> Tuple[str, float]:
        """Detects the spoken language from a NumPy audio array."""
    ...
    def transcribe_chunks(
        ...
        """Transcribes audio in-memory using NumPy slicing for chunking."""
        all_segments = []
        full_text = []
        ...
        for idx in range(num_chunks):
            ...
            # Independent chunk processing for stability and memory efficiency
...
class TranscriberGUI:
    ...
    def _run_transcription(self) -> None:
        ...
        """Background worker for the full transcription pipeline."""
        input_file = self.selected_file.get().strip()
        ...
        try:
            ...
            self._log(f"Device={profile.device}, fp16={profile.fp16}, "
            ...
            # Optimization: Load audio directly to memory to avoid disk I/O
            audio = AudioPreprocessor.load_audio_to_numpy(input_file, self.silence_var.get())
            ...
            language = (None if self.input_lang_override_var.get() == "auto"
