...
class AudioPreprocessor:
    ...
    def preprocess_to_memory(input_path: str, smart_silence: bool) -> np.ndarray:
        """Pipes FFmpeg output directly to a NumPy array to avoid temporary files."""
        silence_filter = [
        ...
        # We capture stdout as raw PCM; stderr is kept for observability
        process = subprocess.run(
            cmd, check=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE
        )
        # Create a copy to ensure the array is writable for PyTorch
        return np.frombuffer(process.stdout, dtype=np.float32).copy()

    @staticmethod
    def chunk_audio_memory(audio: np.ndarray, chunk_seconds: int) -> List[np.ndarray]:
...
class WhisperEngine:
    ...
    def detect_language(self, audio: np.ndarray) -> Tuple[str, float]:
        """Detects language from a NumPy audio buffer."""
...
class TranscriberGUI:
    ...
    def _run_transcription(self) -> None:
        ...
        try:
            ...
            audio_buffer = AudioPreprocessor.preprocess_to_memory(input_file, self.silence_var.get())

            self._set_progress(0.12, "Detecting language")
            detected_lang, lang_prob = engine.detect_language(audio_buffer)
            ...
            chunks = AudioPreprocessor.chunk_audio_memory(audio_buffer, chunk_secs)
