...
class AudioPreprocessor:
    ...
    def preprocess_to_memory(input_path: str, smart_silence: bool) -> np.ndarray:
        """Pipes FFmpeg output directly to a NumPy array in memory (16kHz mono float32)."""
        silence_filter = (
            ["-af", "silenceremove=stop_periods=-1:stop_duration=0.6:stop_threshold=-45dB"]
            if smart_silence else []
        )
        # We use -f f32le to get raw float32 PCM data
        cmd = [
        ...
        # Using stdout=PIPE to capture the raw audio bytes
        result = subprocess.run(cmd, check=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        # np.frombuffer creates a read-only view; .copy() makes it writable for Whisper/Torch
...
class WhisperEngine:
    ...
    def detect_language(self, audio: np.ndarray) -> Tuple[str, float]:
        """Detects language from in-memory audio array."""
