...
class AudioPreprocessor:
    ...
    def preprocess_to_memory(input_path: str, smart_silence: bool) -> np.ndarray:
        """
        Convert input audio to 16kHz mono float32 PCM in memory.
        """
        silence_filter = (
        ...
        # Use subprocess to pipe raw PCM data to stdout
        out = subprocess.check_output(cmd, stderr=subprocess.PIPE)
        # Convert raw S16LE PCM to normalized float32
        return np.frombuffer(out, np.int16).flatten().astype(np.float32) / 32768.0

    @staticmethod
    def chunk_audio_in_memory(audio: np.ndarray, chunk_seconds: int) -> List[np.ndarray]:
        """
        Split normalized audio array into chunks of specified duration.
        """
        sample_rate = 16000
...
class WhisperEngine:
    ...
    def detect_language(self, audio: np.ndarray) -> Tuple[str, float]:
        """
        Detect language from a NumPy audio array.
        """
    ...
    def transcribe_chunks(
        ...
        """
        Transcribe audio chunks (NumPy arrays) and merge results.
        """
        all_segments = []
        full_text = []
...
class TranscriberGUI:
    ...
    def _run_transcription(self) -> None:
        ...
        try:
            ...
            chunks = AudioPreprocessor.chunk_audio_in_memory(audio_array, chunk_seconds)
