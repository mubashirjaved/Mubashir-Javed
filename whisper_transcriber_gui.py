...
class AudioPreprocessor:
    ...
    def preprocess_to_memory(input_path: str, smart_silence: bool) -> np.ndarray:
        """
        Uses FFmpeg to pipe raw PCM data into a NumPy array, avoiding temporary files.
        Returns a float32 array normalized to [-1, 1].
        """
        ...
        # Using stdout=subprocess.PIPE to capture the PCM stream
...
class WhisperEngine:
    ...
    def detect_language(self, audio: np.ndarray) -> Tuple[str, float]:
        """Detects language directly from in-memory audio array."""
    ...
    def transcribe_chunks(
        ...
        """Transcribes audio using in-memory slicing, eliminating redundant FFmpeg calls."""
        all_segments = []
        full_text = []
        ...
        for idx in range(total_chunks):
            ...
            end_sample = min((idx + 1) * samples_per_chunk, len(audio_data))
...
class TranscriberGUI:
    ...
    def _run_transcription(self) -> None:
        ...
        try:
            ...
            total_chunks = math.ceil(len(audio_data) / (self.chunk_var.get() * 16000))
            self._log(f"Prepared {total_chunks} chunk(s) in memory")
