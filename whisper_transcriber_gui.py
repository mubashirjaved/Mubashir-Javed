...
class AudioPreprocessor:
    ...
    def preprocess_to_memory(input_path: str, smart_silence: bool) -> np.ndarray:
        """FFmpeg-based in-memory audio preprocessing."""
        silence_filter = (
        ...
        # Pipe raw float32 PCM data directly to stdout to eliminate temporary files.
        result = subprocess.run(cmd, capture_output=True, check=True)
        # Convert buffer to numpy array and ensure it is writable for Torch/Whisper compatibility.
...
class WhisperEngine:
    ...
    def detect_language(self, audio: np.ndarray) -> Tuple[str, float]:
        """Detect language from in-memory audio array."""
    ...
    def transcribe_chunks(
        ...
        """Transcribe in-memory audio in chunks to optimize performance and stability."""
        all_segments = []
        full_text = []
        sample_rate = 16000
        chunk_samples = chunk_seconds * sample_rate
        total_chunks = math.ceil(len(audio) / chunk_samples) if len(audio) > 0 else 1
        ...
        for idx in range(total_chunks):
            ...
            end_sample = min((idx + 1) * chunk_samples, len(audio))
            audio_chunk = audio[start_sample:end_sample]

            progress_cb(idx / total_chunks, f"Transcribing chunk {idx + 1}/{total_chunks}")
        ...
        merged_text = re.sub(r"\s+", " ", " ".join(full_text)).strip()
        if not timestamps:
...
class TranscriberGUI:
    ...
    def _run_transcription(self) -> None:
        ...
        try:
            ...
            language = (
                None
                if self.input_lang_override_var.get() == "auto"
            ...
            result = engine.transcribe_chunks(
                ...
                progress_cb=lambda p, msg: self._set_progress(0.2 + 0.8 * p, msg),
            )
