...
class AudioPreprocessor:
    ...
    def preprocess_to_memory(input_path: str, smart_silence: bool) -> np.ndarray:
        """Pipes raw PCM data from FFmpeg directly into a NumPy array, skipping disk I/O."""
        ...
        # Use stdout=subprocess.PIPE to capture raw bytes
        proc = subprocess.run(cmd, check=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        # Normalize 16-bit PCM to [-1, 1] float32 as expected by Whisper
    ...
    def chunk_audio_in_memory(audio: np.ndarray, chunk_seconds: int) -> List[np.ndarray]:
        """Slices a NumPy array into chunks of specified duration using O(1) slicing."""
...
class WhisperEngine:
    ...
    def detect_language(self, audio: np.ndarray) -> Tuple[str, float]:
        """Detects language from a NumPy array. Whisper's pad_or_trim and log_mel_spectrogram handle arrays."""
    ...
    def transcribe_chunks(
        ...
        """Transcribes multiple audio chunks (NumPy arrays) and merges segments with correct time offsets."""
        all_segments = []
        full_text = []
...
class TranscriberGUI:
    ...
    def _run_transcription(self) -> None:
        ...
        try:
            ...
            chunks = AudioPreprocessor.chunk_audio_in_memory(audio_mem, chunk_seconds)
