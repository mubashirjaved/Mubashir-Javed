...
class AudioPreprocessor:
    ...
    def preprocess_to_memory(input_path: str, smart_silence: bool) -> np.ndarray:
        """Loads and normalizes audio to a float32 NumPy array at 16kHz mono in-memory."""
        ...
        # Convert buffer to NumPy array (int16 -> float32 normalized)
        audio = np.frombuffer(out, np.int16).astype(np.float32) / 32768.0
        return audio

    @staticmethod
    def chunk_audio_memory(audio: np.ndarray, chunk_seconds: int, sample_rate: int = 16000) -> List[np.ndarray]:
        """Slices a large audio array into smaller chunks without re-processing."""
...
class TranscriberGUI:
    ...
    def _run_transcription(self) -> None:
        ...
        try:
            ...
            chunks = AudioPreprocessor.chunk_audio_memory(audio_data, chunk_sec)
