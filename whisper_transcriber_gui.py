...
class AudioPreprocessor:
    """Handles audio preprocessing and chunking using FFmpeg and NumPy."""

    @staticmethod
    def preprocess_to_memory(input_path: str, smart_silence: bool) -> np.ndarray:
        """Pipes FFmpeg output directly to a NumPy array for in-memory processing."""
        silence_filter = (
        ...
        # In-memory optimization: avoid disk I/O by piping raw PCM to stdout
        out = subprocess.run(cmd, check=True, capture_output=True).stdout
        return np.frombuffer(out, np.float32).copy()

    @staticmethod
    def chunk_audio_numpy(audio_array: np.ndarray, chunk_seconds: int) -> List[np.ndarray]:
        """Slices a NumPy array into chunks of specified duration."""
        sample_rate = 16000
        chunk_samples = chunk_seconds * sample_rate
        chunks = []
        # O(1) slicing for each chunk instead of expensive FFmpeg process spawns
...
class WhisperEngine:
    """Wraps OpenAI Whisper model for optimized transcription on target hardware."""
    ...
    def detect_language(self, audio_data: np.ndarray) -> Tuple[str, float]:
        """Detects spoken language from in-memory audio data."""
        # Use in-memory data directly to bypass redundant file loading
    ...
    def transcribe_chunks(
        ...
        """Transcribes audio chunks using the loaded Whisper model."""
        all_segments = []
        full_text = []
        ...
        for idx, chunk in enumerate(chunks):
            ...
            # model.transcribe accepts NumPy array directly
...
class TranscriberGUI:
    ...
    def _run_transcription(self) -> None:
        ...
        try:
            ...
            # In-memory pipeline: Preprocess directly to NumPy array
            ...
            chunks = AudioPreprocessor.chunk_audio_numpy(audio_data, chunk_seconds)
            self._log(f"Prepared {len(chunks)} chunk(s) using in-memory slicing")
