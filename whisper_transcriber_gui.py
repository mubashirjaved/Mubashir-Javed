...
class AudioPreprocessor:
    """Handles audio preprocessing and in-memory loading using FFmpeg."""

    @staticmethod
    def preprocess_to_memory(input_path: str, smart_silence: bool) -> np.ndarray:
        """
        Preprocesses audio using FFmpeg and returns a float32 numpy array (16kHz mono).
        Avoids intermediate disk I/O by piping raw PCM data from FFmpeg to stdout.
        """
        silence_filter = ["-af", "silenceremove=stop_periods=-1:stop_duration=0.6:stop_threshold=-45dB"] if smart_silence else []
        # We use f32le (float 32-bit little endian) for direct compatibility with Whisper/NumPy
        cmd = [
        ...
        # capture_output=True captures stdout and stderr to ensure errors are observable
...
class WhisperEngine:
    """Core transcription engine using OpenAI Whisper with in-memory optimizations."""
    ...
    def detect_language(self, audio: np.ndarray) -> Tuple[str, float]:
        """Detects language from an in-memory audio array."""
    ...
    def transcribe_chunks(
        ...
        """
        Transcribes audio in-memory by slicing the numpy array into chunks.
        Avoids redundant disk writes and FFmpeg calls.
        """
        sample_rate = 16000
