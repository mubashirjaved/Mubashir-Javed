"""Whisper Audio Transcriber GUI with in-memory pipeline optimization."""
import math
import os
import queue
import re
...
class AudioPreprocessor:
    """Handles audio loading and segmenting in memory using FFmpeg and NumPy."""
    @staticmethod
    def preprocess_to_memory(input_path: str, smart_silence: bool) -> np.ndarray:
        """Loads audio and pre-processes it to 16kHz mono float32 in memory."""
        ...
        # Convert s16le to float32 and normalize to [-1.0, 1.0]
        return np.frombuffer(result.stdout, np.int16).flatten().astype(np.float32) / 32768.0

    @staticmethod
    def chunk_audio_memory(audio_data: np.ndarray, chunk_seconds: int) -> List[np.ndarray]:
        """Slices NumPy audio data into chunks of specified duration."""
        sample_rate = 16000
...
class WhisperEngine:
    ...
    def transcribe_chunks(
        ...
        """Transcribes multiple audio chunks in memory and merges results."""
        all_segments = []
        full_text = []
...
class TranscriberGUI:
    ...
    def _run_transcription(self) -> None:
        ...
        try:
            ...
            chunks = AudioPreprocessor.chunk_audio_memory(audio_data, chunk_secs)
