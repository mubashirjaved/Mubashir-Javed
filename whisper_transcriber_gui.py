...
class AudioPreprocessor:
    ...
    def preprocess_to_memory(input_path: str, smart_silence: bool) -> np.ndarray:
        """Pipes audio from FFmpeg to a NumPy array in memory."""
    ...
    @staticmethod
    def chunk_audio(audio_array: np.ndarray, chunk_seconds: int) -> List[np.ndarray]:
        """Slices a NumPy array into chunks of specified duration."""
        chunk_samples = chunk_seconds * 16000
        return [audio_array[i : i + chunk_samples] for i in range(0, len(audio_array), chunk_samples)]
...
class WhisperEngine:
    ...
    def transcribe_chunks(
        ...
        """Transcribes audio chunks in memory and merges results."""
        all_segments = []
        full_text = []
...
class TranscriberGUI:
    ...
    def _run_transcription(self) -> None:
        ...
        try:
            ...
            self._set_progress(0.18, "Chunking in memory")
            chunks = AudioPreprocessor.chunk_audio(audio_array, chunk_seconds)
