...
class AudioPreprocessor:
    ...
    def preprocess_to_memory(input_path: str, smart_silence: bool) -> np.ndarray:
        """
        Uses FFmpeg to pipe raw f32le PCM data directly to memory, eliminating temp WAV files.
        """
        silence_filter = (
        ...
        # Capture raw float32 PCM data from stdout
        out = subprocess.run(cmd, check=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE).stdout
        # np.frombuffer returns a read-only view. .copy() ensures it is writable for Torch.
...
class WhisperEngine:
    ...
    def transcribe_chunks(
        ...
        """
        Transcribes audio in chunks to optimize memory and handle long files.
        Uses NumPy slicing for zero-copy chunking.
        """
        sample_rate = 16000
        ...
        for idx in range(num_chunks):
            ...
            chunk_offset_sec = idx * chunk_seconds
            for seg in result.get("segments", []):
                seg = dict(seg)
                seg["start"] = seg.get("start", 0.0) + chunk_offset_sec
                seg["end"] = seg.get("end", 0.0) + chunk_offset_sec
