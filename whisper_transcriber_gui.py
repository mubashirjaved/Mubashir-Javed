...
class AudioPreprocessor:
    ...
    def preprocess_to_memory(input_path: str, smart_silence: bool) -> np.ndarray:
        """Optimized: Preprocesses audio directly to memory using FFmpeg pipes and NumPy."""
        ...
        # Redirect stderr to DEVNULL to avoid potential buffer deadlocks
        process = subprocess.run(cmd, stdout=subprocess.PIPE, stderr=subprocess.DEVNULL, check=True)
...
class WhisperEngine:
    ...
    def detect_language(self, audio: np.ndarray) -> Tuple[str, float]:
        ...
        mel = whisper.log_mel_spectrogram(audio.flatten()).to(self.model.device)
    ...
    def transcribe_chunks(
        ...
        for i in range(num_chunks):
            ...
            chunk_offset = start_sample / 16000.0
