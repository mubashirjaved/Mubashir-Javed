...
class AudioPreprocessor:
    ...
    def preprocess_to_memory(input_path: str, smart_silence: bool) -> np.ndarray:
        """Loads audio from disk and processes it into a 16kHz mono float32 NumPy array."""
        ...
        # In-memory processing via FFmpeg pipe:1 eliminates temporary WAV files.
        process = subprocess.run(cmd, check=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        # Use .copy() to ensure array is writable, avoiding Torch UserWarnings.
...
class WhisperEngine:
    ...
    def detect_language(self, audio: np.ndarray) -> Tuple[str, float]:
        """Detects language from an in-memory audio array, bypassing disk I/O."""
    ...
    def transcribe_chunks(
        ...
        """Transcribes audio in in-memory chunks, using NumPy slicing for zero-overhead chunking."""
        all_segments = []
        full_text = []
...
class TranscriberGUI:
    ...
    def _run_transcription(self) -> None:
        ...
        try:
            ...
            # In-memory pipeline eliminates temporary files and redundant disk I/O.
