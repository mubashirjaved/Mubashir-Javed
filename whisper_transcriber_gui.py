...
class AudioPreprocessor:
    ...
    def preprocess_to_memory(input_path: str, smart_silence: bool) -> np.ndarray:
        """Decodes and pre-processes audio into a normalized float32 NumPy array."""
        silence_filter = (
            ["-af", "silenceremove=stop_periods=-1:stop_duration=0.6:stop_threshold=-45dB"]
            if smart_silence else []
        )
        # Use f32le for raw PCM float32 output at 16kHz mono.
        cmd = [
            "ffmpeg", "-y", "-hwaccel", "auto", "-i", input_path,
            "-f", "f32le", "-ac", "1", "-ar", "16000",
            *silence_filter, "-"
        ]
        # Execute FFmpeg and capture stdout as a bytes buffer.
        result = subprocess.run(cmd, check=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        # Convert buffer to NumPy array and ensure it is a writable copy.
        return np.frombuffer(result.stdout, np.float32).copy()

    @staticmethod
    def chunk_audio(audio_data: np.ndarray, chunk_seconds: int) -> List[np.ndarray]:
        """Slices a large audio array into smaller chunks in-memory."""
        sample_rate = 16000
...
class WhisperEngine:
    ...
    def transcribe_chunks(
        ...
        """Transcribes audio chunks and merges the results with correct offsets."""
        all_segments = []
        full_text = []
        ...
        for idx, chunk in enumerate(chunks):
            ...
            for seg in result.get("segments", []):
                ...
                # Correct word timestamps if present
...
class TranscriberGUI:
    ...
    def _run_transcription(self) -> None:
        ...
        try:
            ...
            # In-memory preprocessing eliminates intermediate WAV files.
            ...
            chunks = AudioPreprocessor.chunk_audio(audio_data, chunk_sec)
