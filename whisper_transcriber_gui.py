...
class AudioPreprocessor:
    """Handles audio loading and chunking in memory for performance."""
    @staticmethod
    def preprocess_to_memory(input_path: str, smart_silence: bool) -> np.ndarray:
        ...
        # Output as raw f32le PCM to pipe
        cmd = [
    ...
    def chunk_audio_memory(audio_data: np.ndarray, chunk_seconds: int) -> List[np.ndarray]:
        """Slices a NumPy array into chunks without disk I/O."""
        sample_rate = 16000
        ...
        for i in range(0, len(audio_data), chunk_samples):
            chunks.append(audio_data[i : i + chunk_samples])
        return chunks
...
class WhisperEngine:
    ...
    def detect_language(self, audio_data: np.ndarray) -> Tuple[str, float]:
        """Detects language from in-memory NumPy audio data."""
    ...
    def transcribe_chunks(
        ...
        """Transcribes audio chunks from memory with correct timestamp offsets."""
        all_segments = []
        full_text = []
...
class TranscriberGUI:
    ...
    def _run_transcription(self) -> None:
        ...
        try:
            ...
            self._set_progress(0.15, "Chunking in memory")
