...
class AudioPreprocessor:
    ...
    def preprocess_to_memory(input_path: str, smart_silence: bool) -> np.ndarray:
        """Fetch 16kHz mono float32 PCM data directly into a numpy array."""
...
class WhisperEngine:
    ...
    def detect_language(self, audio_data: np.ndarray) -> Tuple[str, float]:
        """Detect language directly from pre-loaded float32 PCM data."""
    ...
    def transcribe_chunks(
        ...
        """In-memory chunked transcription to minimize process overhead."""
        all_segments = []
        full_text = []
        ...
        # Join chunks while ensuring no double spaces, as Whisper segments often lead with a space.
        merged_text = " ".join(full_text).replace("  ", " ").strip()
        if not timestamps:
