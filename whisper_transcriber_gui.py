...
class AudioPreprocessor:
    ...
    def preprocess_to_memory(input_path: str, smart_silence: bool) -> np.ndarray:
        """Pipes preprocessed audio directly to a numpy array, eliminating temporary files."""
        silence_filter = (
...
class WhisperEngine:
    ...
    def __init__(self, profile: HardwareProfile, model_name: str):
        ...
        if cache_key not in WhisperEngine._model_cache:
            WhisperEngine._model_cache[cache_key] = whisper.load_model(
                model_name, device=profile.device
            )
        self.model = WhisperEngine._model_cache[cache_key]

    def detect_language(self, audio: np.ndarray) -> Tuple[str, float]:
        """Detects language from an in-memory numpy array."""
...
class TranscriberGUI:
    ...
    def _run_transcription(self) -> None:
        ...
        try:
            ...
            audio_data = AudioPreprocessor.preprocess_to_memory(
                input_file, self.silence_var.get()
            )
