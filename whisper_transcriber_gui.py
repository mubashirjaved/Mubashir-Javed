...
class AudioPreprocessor:
    ...
    def preprocess_to_memory(input_path: str, smart_silence: bool) -> np.ndarray:
        """
        Preprocesses audio to 16kHz mono float32 PCM and returns as a NumPy array.
        Uses FFmpeg pipe to avoid temporary files and redundant I/O.
        """
        silence_filter = [
            "-af",
            "silenceremove=stop_periods=-1:stop_duration=0.6:stop_threshold=-45dB"
        ] if smart_silence else []

        # Base FFmpeg command for 16kHz mono float32 PCM
        cmd = [
            "ffmpeg", "-i", input_path,
            "-ac", "1", "-ar", "16000",
        ...
        # Use -hwaccel auto for potential hardware decoding speedup
        hw_cmd = ["ffmpeg", "-hwaccel", "auto"] + cmd[1:]
...
class WhisperEngine:
    ...
    def detect_language(self, audio_data: np.ndarray) -> Tuple[str, float]:
        """Detects the spoken language from the start of the audio data."""
    ...
    def transcribe_chunks(
        ...
        """Transcribes audio data in chunks to manage memory and provide progress updates."""
        all_segments = []
        full_text = []

        # 16kHz sample rate as per Whisper requirements
        ...
        for idx in range(total_chunks):
            ...
            end_sample = min((idx + 1) * chunk_samples, len(audio_data))
            ...
            # Accurate offset calculation using dynamic chunk_seconds
...
class TranscriberGUI:
    ...
    def _run_transcription(self) -> None:
        ...
        try:
            ...
            self._set_progress(0.2, "Starting transcription")
