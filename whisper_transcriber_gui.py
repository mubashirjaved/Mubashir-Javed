...
class AudioPreprocessor:
    ...
    def preprocess_to_memory(input_path: str, smart_silence: bool) -> np.ndarray:
        """Preprocess audio using FFmpeg and return a normalized NumPy array."""
        silence_filter = ["-af", "silenceremove=stop_periods=-1:stop_duration=0.6:stop_threshold=-45dB"] if smart_silence else []
        # We use s16le for compatibility and manual normalization, matching whisper.load_audio
        cmd = [
...
class WhisperEngine:
    ...
    def detect_language(self, audio: np.ndarray) -> Tuple[str, float]:
        """Detect language from preprocessed audio NumPy array."""
    ...
    def transcribe_chunks(
        ...
        """Transcribe audio in memory-mapped chunks to avoid disk I/O."""
        all_segments = []
        full_text = []
        sr = 16000
