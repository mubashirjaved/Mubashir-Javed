...
class AudioPreprocessor:
    ...
    def preprocess_to_memory(input_path: str, smart_silence: bool) -> np.ndarray:
        """Preprocesses audio directly into a NumPy array via FFmpeg pipe."""
        silence_filter = (
            ["-af", "silenceremove=stop_periods=-1:stop_duration=0.6:stop_threshold=-45dB"]
            if smart_silence
            else []
        )
        # We use s16le for the pipe and convert to float32 to match whisper.load_audio behavior
        cmd = [
...
class WhisperEngine:
    """Wrapper around OpenAI Whisper for optimized transcription."""
    ...
    def detect_language(self, audio: np.ndarray) -> Tuple[str, float]:
        """Detects the spoken language from audio array."""
    ...
    def transcribe_chunks(
        ...
        for idx in range(num_chunks):
            ...
            for seg in result.get("segments", []):
                ...
                if "words" in seg:
                    for w in seg["words"]:
                        w["start"] += chunk_offset_seconds
                        w["end"] += chunk_offset_seconds
