...
class AudioPreprocessor:
    """Handles audio decoding and preprocessing tasks."""

    @staticmethod
    def preprocess_to_memory(input_path: str, smart_silence: bool) -> np.ndarray:
        """Decodes audio to a 16kHz mono float32 NumPy array using FFmpeg pipes."""
...
class WhisperEngine:
    ...
    def detect_language(self, audio: np.ndarray) -> Tuple[str, float]:
        """Detects the spoken language from a preprocessed NumPy audio array."""
    ...
    def transcribe_chunks(
        ...
        """Transcribes audio in in-memory chunks to optimize speed and memory usage."""
        all_segments = []
        full_text = []

        # Whisper expects 16kHz audio
        samples_per_chunk = chunk_seconds * 16000
        num_chunks = math.ceil(len(audio) / samples_per_chunk)
        ...
        for idx in range(num_chunks):
            ...
            chunk_offset_seconds = idx * chunk_seconds
            for seg in result.get("segments", []):
                seg = dict(seg)
                seg["start"] = seg.get("start", 0.0) + chunk_offset_seconds
                seg["end"] = seg.get("end", 0.0) + chunk_offset_seconds
...
class TranscriberGUI:
    ...
    def _run_transcription(self) -> None:
        """Main background worker for transcription orchestration."""
        input_file = self.selected_file.get().strip()
