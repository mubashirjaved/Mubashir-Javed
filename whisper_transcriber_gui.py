...
# Ensure FFmpeg binaries are in the system PATH for subprocess calls
add_paths()
...
class AudioPreprocessor:
    ...
    def preprocess_to_memory(input_path: str, smart_silence: bool) -> np.ndarray:
        """
        Optimized preprocessing: Pipes raw PCM data from FFmpeg directly to a NumPy array.
        Eliminates slow disk I/O by avoiding intermediate temporary WAV files.
        """
        ...
        # Convert raw S16LE bytes to normalized float32 NumPy array
    ...
    def chunk_audio_in_memory(audio: np.ndarray, chunk_seconds: int, sample_rate: int = 16000) -> List[np.ndarray]:
        """
        Fast in-memory chunking using NumPy slicing instead of FFmpeg subprocesses.
        """
...
class TextPostProcessor:
    """
    Optimized text cleanup using pre-compiled regex patterns to reduce processing overhead.
    """
    ...
    def cleanup_text(text: str) -> str:
        text = TextPostProcessor._FILLER_PATTERN.sub("", text)
        text = TextPostProcessor._REPETITION_PATTERN.sub(r"\1", text)
        text = TextPostProcessor._WHITESPACE_PATTERN.sub(" ", text).strip()
        return text


class WhisperEngine:
    """
    Whisper inference engine with class-level model caching to prevent redundant
    multi-second model loading times on repeated transcription runs.
    """
    ...
    def __init__(self, profile: HardwareProfile, model_name: str):
        ...
        if cache_key not in WhisperEngine._model_cache:
            WhisperEngine._model_cache[cache_key] = whisper.load_model(model_name, device=profile.device)
        self.model = WhisperEngine._model_cache[cache_key]
...
class TranscriberGUI:
    ...
    def _run_transcription(self) -> None:
        ...
        try:
            ...
            chunk_secs = self.chunk_var.get()
            chunks = AudioPreprocessor.chunk_audio_in_memory(audio_data, chunk_secs)
