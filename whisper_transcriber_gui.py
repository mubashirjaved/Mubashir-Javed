...
class AudioPreprocessor:
    ...
    def preprocess_to_memory(input_path: str, smart_silence: bool) -> np.ndarray:
        ...
        # Use stdout=subprocess.PIPE to capture raw PCM data, redirect stderr to DEVNULL for cleaner logs
        result = subprocess.run(cmd, stdout=subprocess.PIPE, stderr=subprocess.DEVNULL, check=True)
        # Convert PCM S16LE to float32 normalized to [-1, 1] as expected by Whisper
