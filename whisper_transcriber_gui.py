...
class AudioPreprocessor:
    ...
    def preprocess_to_memory(input_path: str, smart_silence: bool) -> np.ndarray:
        """Loads audio and preprocesses it to a NumPy array in memory."""
        silence_filter = (
        ...
        # Use subprocess.run to capture the raw PCM float32 data from stdout
        result = subprocess.run(cmd, capture_output=True, check=True)
        # Convert buffer to NumPy array and copy to ensure it is writable
    ...
    def transcribe_chunks(
        ...
        for idx in range(num_chunks):
            ...
            for seg in result.get("segments", []):
                ...
                # Also offset word timestamps if they exist
...
class TranscriberGUI:
    ...
    def _run_transcription(self) -> None:
        ...
        try:
            ...
            # In-memory pipeline handles chunking during transcription
            chunk_seconds = self.chunk_var.get()
            self._log(f"Transcription starting (chunk_size={chunk_seconds}s)")
