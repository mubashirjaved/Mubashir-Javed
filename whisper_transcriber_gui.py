...
class AudioPreprocessor:
    ...
    def preprocess_to_memory(input_path: str, smart_silence: bool) -> np.ndarray:
        """Preprocess audio to an in-memory numpy array (16kHz mono float32)."""
        silence_filter = (
        ...
        # Bolt: In-memory pipeline reduces disk I/O latency and eliminates redundant FFmpeg calls.
...
class WhisperEngine:
    ...
    def detect_language(self, audio_data: np.ndarray) -> Tuple[str, float]:
        """Detect language using an in-memory audio array."""
    ...
    def transcribe_chunks(
        ...
        """Transcribe full audio in-memory using slices to simulate chunking."""
        all_segments = []
        full_text = []
        ...
        for idx in range(total_chunks):
            ...
            # Bolt: In-memory slicing is O(1) and avoids redundant FFmpeg processing/disk reads.
