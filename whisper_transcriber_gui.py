...
class AudioPreprocessor:
    ...
    def preprocess_to_memory(input_path: str, smart_silence: bool) -> np.ndarray:
        """
        Loads audio from any format into a 1D float32 numpy array at 16kHz mono.
        Eliminates intermediate disk writes and redundant FFmpeg calls.
        """
        silence_filter = (
        ...
        # Using .copy() because np.frombuffer returns a read-only view, and Whisper might need writability.
...
class WhisperEngine:
    ...
    def detect_language(self, audio: np.ndarray) -> Tuple[str, float]:
        """Identifies the spoken language in the first 30 seconds of the provided audio."""
    ...
    def transcribe_chunks(
        ...
        """Transcribes audio in sequential segments using in-memory slicing for performance."""
        all_segments = []
        full_text = []

        # Whisper expects 16,000 samples per second.
        sample_rate = 16000
