# pylint: disable=import-error,no-member,attribute-defined-outside-init,line-too-long,missing-docstring,broad-exception-caught,invalid-name,too-few-public-methods,too-many-arguments,too-many-positional-arguments,too-many-locals,too-many-instance-attributes
import math
import os
import queue
import re
...
class AudioPreprocessor:
    ...
    def preprocess_to_memory(input_path: str, smart_silence: bool) -> np.ndarray:
        """
        Converts input audio to in-memory numpy array (16kHz, float32, mono) using FFmpeg.
        This eliminates the need for temporary WAV files and reduces disk I/O.
        """
        ...
        # Pipe raw PCM data to stdout and load it into a numpy array
...
class WhisperEngine:
    ...
    def detect_language(self, audio: np.ndarray) -> Tuple[str, float]:
        """Detects the language of the audio data using the Whisper model."""
    ...
    def transcribe_chunks(
        ...
        """
        Transcribes the audio in chunks using in-memory numpy slicing.
        Fixes hardcoded 180s offset and eliminates temporary file overhead.
        """
        all_segments = []
        full_text = []

        samples_per_chunk = chunk_seconds * 16000
        num_chunks = max(1, math.ceil(len(full_audio) / samples_per_chunk))
        ...
        for idx in range(num_chunks):
            ...
            end_sample = min((idx + 1) * samples_per_chunk, len(full_audio))
...
class TranscriberGUI:
    ...
    def _run_transcription(self) -> None:
        ...
        try:
            ...
            full_audio = AudioPreprocessor.preprocess_to_memory(input_file, self.silence_var.get())

            self._set_progress(0.12, "Detecting language")
            detected_lang, lang_prob = engine.detect_language(full_audio)
