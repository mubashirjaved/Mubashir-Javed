...
class AudioPreprocessor:
    ...
    def preprocess_to_memory(input_path: str, smart_silence: bool) -> np.ndarray:
        """
        Converts audio to 16kHz mono float32 PCM data in memory.
        ...
        # Pipe raw PCM data from stdout
        process = subprocess.run(cmd, capture_output=True, check=True)
        # Convert 16-bit PCM to float32 normalized between -1 and 1
...
class WhisperEngine:
    ...
    def transcribe_chunks(
        ...
        for idx in range(num_chunks):
            ...
            end_sample = min((idx + 1) * chunk_size, total_samples)
            ...
            chunk_offset = (idx * chunk_size) / sample_rate
            for seg in result.get("segments", []):
                ...
                if "words" in seg:
                    for word in seg["words"]:
                        word["start"] = word.get("start", 0.0) + chunk_offset
                        word["end"] = word.get("end", 0.0) + chunk_offset
