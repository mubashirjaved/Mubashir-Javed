...
class AudioPreprocessor:
    ...
    def preprocess_to_memory(input_path: str, smart_silence: bool) -> np.ndarray:
        """
        Uses FFmpeg to pipe raw float32 PCM (f32le) data directly to a numpy array.
        Eliminates intermediate WAV files and redundant disk I/O.
        """
        ...
        # We capture stdout for the raw PCM data and stderr for any FFmpeg logs/errors.
        result = subprocess.run(cmd, check=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        # np.frombuffer returns a read-only view; .copy() ensures it is writable for PyTorch.
...
class WhisperEngine:
    ...
    def detect_language(self, audio: np.ndarray) -> Tuple[str, float]:
        """
        Identifies the spoken language from the pre-loaded numpy array.
        """
    ...
    def transcribe_chunks(
        ...
        """
        Transcribes the full audio array by slicing it into memory-resident chunks.
        Eliminates the need for temporary chunk files and redundant FFmpeg re-loads.
        """
        all_segments = []
        full_text = []
        ...
        for i in range(num_chunks):
            ...
            progress_cb(i / num_chunks, f"Transcribing chunk {i + 1}/{num_chunks}")
