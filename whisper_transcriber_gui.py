...
class AudioPreprocessor:
    ...
    def preprocess_to_memory(input_path: str, smart_silence: bool) -> np.ndarray:
        """
        Preprocesses audio directly into a NumPy array using FFmpeg.
        Avoids temporary files and redundant disk I/O.
        """
        ...
        # Use stdout=PIPE to capture the PCM data
        result = subprocess.run(cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE, check=True)
        # Convert raw S16LE PCM to float32 normalized to [-1, 1]
...
class WhisperEngine:
    ...
    def detect_language(self, audio: np.ndarray) -> Tuple[str, float]:
        """
        Detects the language of the audio array.
        Expects a 1D float32 NumPy array normalized to [-1, 1].
        """
    ...
    def transcribe_chunks(
        ...
        """
        Transcribes the audio array in chunks to manage memory and provide progress updates.
        Slices the array in-memory, avoiding temporary files.
        """
        all_segments = []
        full_text = []
        sample_rate = 16000
        chunk_samples = chunk_seconds * sample_rate
        total_chunks = math.ceil(len(full_audio) / chunk_samples)
        ...
        for idx in range(total_chunks):
            ...
            end_sample = min((idx + 1) * chunk_samples, len(full_audio))
...
class TranscriberGUI:
    ...
    def _run_transcription(self) -> None:
        ...
        try:
            ...
            # Fully in-memory pipeline: Preprocess -> Detected -> Transcribe (via slicing)
            audio_mem = AudioPreprocessor.preprocess_to_memory(input_file, self.silence_var.get())

            self._set_progress(0.12, "Detecting language")
            detected_lang, lang_prob = engine.detect_language(audio_mem)
