...
class AudioPreprocessor:
    """Handles audio decoding and preprocessing using FFmpeg."""
    @staticmethod
    def preprocess_to_memory(input_path: str, smart_silence: bool) -> np.ndarray:
        """
        ⚡ Bolt Optimization: Uses FFmpeg to pipe raw PCM data directly to memory.
        This eliminates intermediate WAV file creation and reduces disk I/O overhead.
        """
    ...
    def chunk_audio_memory(audio_data: np.ndarray, chunk_seconds: int) -> List[np.ndarray]:
        """
        ⚡ Bolt Optimization: Slices the NumPy array for O(1) time complexity chunking.
        This replaces multiple FFmpeg subprocess spawns with simple memory views.
        """
        sample_rate = 16000
...
class TranscriberGUI:
    ...
    def _run_transcription(self) -> None:
        ...
        try:
            ...
            self._set_progress(0.18, "Chunking")
            chunk_seconds = self.chunk_var.get()
            chunks = AudioPreprocessor.chunk_audio_memory(audio_data, chunk_seconds)
