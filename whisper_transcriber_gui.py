...
class AudioPreprocessor:
    ...
    def preprocess_to_memory(input_path: str, smart_silence: bool) -> np.ndarray:
        """
        Uses FFmpeg to decode and resample audio directly into a float32 numpy array.
        Eliminates the need for temporary WAV files during preprocessing.
        """
        ...
        # Convert raw bytes to numpy array (S16LE is 2 bytes per sample)
        audio = np.frombuffer(process.stdout, dtype=np.int16).flatten().astype(np.float32) / 32768.0
        return audio
    ...
    def chunk_audio(audio: np.ndarray, chunk_seconds: int) -> List[np.ndarray]:
        ...
        Chunking is now instantaneous via array slicing.
        """
        sample_rate = 16000
...
class TranscriberGUI:
    ...
    def _run_transcription(self) -> None:
        ...
        try:
            ...
            # Preprocess directly to memory (numpy array)
