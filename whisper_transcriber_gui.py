"""
Whisper Audio Transcriber GUI with performance optimizations.
Uses in-memory processing to avoid redundant disk I/O.
"""
...
class HardwareOptimizer:
    """Helper for detecting hardware and recommending optimal settings."""
    @staticmethod
    def detect_amd_gpu() -> Tuple[bool, List[str]]:
        """Probes the system for AMD GPU support via CUDA, ROCm, or OpenCL."""
        notes = []
        amd_found = False
    ...
    def recommend_model(cpu_threads: int, amd_gpu: bool) -> str:
        """Suggests a Whisper model size based on available resources."""
    ...
    def build_profile(thread_limit: Optional[int] = None) -> HardwareProfile:
        """Configures torch and builds a hardware profile."""
        ...
        if not use_cuda:
            ...
            notes.append(
                "AMD GPU found but native Whisper CUDA path may be unavailable; "
                "using hybrid/CPU-safe path."
            )
...
class AudioPreprocessor:
    """Handles audio decoding, resampling, and silencing using FFmpeg."""
...
class TextPostProcessor:
    """Cleans up and normalizes transcribed text."""
    # pylint: disable=too-few-public-methods
    @staticmethod
    def cleanup_text(text: str) -> str:
        """Removes filler words, duplicates, and normalizes whitespace."""
...
class WhisperEngine:
    """Wraps OpenAI Whisper for language detection and transcription."""
    # pylint: disable=too-few-public-methods
    ...
    def detect_language(self, audio: np.ndarray) -> Tuple[str, float]:
        """Detects the dominant language in the provided audio array."""
    ...
    def transcribe_chunks(
        ...
        """Transcribes audio in chunks to manage memory and provide progress updates."""
        all_segments = []
        full_text = []
        sample_rate = 16000
...
def format_srt(segments: List[Dict]) -> str:
    """Formats segments into SubRip (SRT) format."""
    def ts(sec: float) -> str:
        ms_val = int((sec - int(sec)) * 1000)
        h_val = int(sec // 3600)
        m_val = int((sec % 3600) // 60)
        s_val = int(sec % 60)
...
class TranscriberGUI:
    """Tkinter-based GUI for the Whisper transcriber."""
    ...
    def _build_ui(self) -> None:
        ...
        ttk.Combobox(
            top, textvariable=self.input_lang_override_var,
            values=["auto", "en", "ur", "hi", "ar", "fr", "de", "es", "tr", "zh"],
            width=12, state="readonly"
        ).grid(row=1, column=5, sticky="w", **pad)

        ttk.Label(top, text="Output language").grid(row=1, column=6, sticky="w", **pad)
        ttk.Combobox(
            top, textvariable=self.output_lang_var, values=OUTPUT_LANGUAGES,
            width=12, state="readonly"
        ...
        ttk.Button(
            actions, text="Stop", command=self.stop_transcription
        ).pack(side="left", padx=6)
        ttk.Button(
            actions, text="Copy", command=self.copy_output
        ).pack(side="left", padx=6)
        ttk.Button(
    ...
    def _set_recommended_model(self) -> None:
        """Analyzes hardware and sets a recommended Whisper model."""
    ...
    def _set_progress(self, value: float, msg: str) -> None:
        """Updates the progress bar and window title."""
        self.log_queue.put(("progress", (value, msg)))

    def _pump_logs(self) -> None:
        """Polls the logging queue and updates the UI."""
        try:
            while True:
    ...
    def browse_file(self) -> None:
        """Opens a file dialog to select an input media file."""
    ...
    def start_transcription(self) -> None:
        """Starts the transcription process in a background thread."""
    ...
    def _run_transcription(self) -> None:
        """Background worker method that orchestrates the transcription pipeline."""
        input_file = self.selected_file.get().strip()
        ...
        try:
            ...
            self._log(
                f"Device={profile.device}, fp16={profile.fp16}, "
                f"threads={profile.torch_threads}"
            )
            for note in profile.notes:
                self._log(note)
    ...
    def _translate_text(self, text: str, source_lang: str, target_lang: str) -> str:
        """Translates the transcribed text using deep-translator."""
        mapping = {
    ...
    def copy_output(self) -> None:
        """Copies the transcribed text to the system clipboard."""
    ...
    def save_output(self, fmt: str) -> None:
        """Saves the transcription result to a TXT or SRT file."""
        ...
        if fmt == "txt":
            ...
            Path(path).write_text(
                self.last_result.get("text", ""), encoding="utf-8"
            )
