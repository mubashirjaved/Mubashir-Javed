...
try:
    from deep_translator import GoogleTranslator  # pylint: disable=import-error
except Exception:
...
class AudioPreprocessor:
    """Handles audio loading and preprocessing into memory using FFmpeg."""

    @staticmethod
    def preprocess_to_memory(input_path: str, smart_silence: bool) -> np.ndarray:
        """
        Preprocesses audio directly to a NumPy array using FFmpeg pipes.
        Converts to 16kHz mono float32 PCM.
        """
        ...
        # Optimized: Reading raw PCM data from stdout avoids disk I/O for intermediate files.
        # .copy() is used because np.frombuffer returns a read-only view.
...
class WhisperEngine:
    """Wraps OpenAI Whisper model for optimized transcription."""
    ...
    def detect_language(self, audio: np.ndarray) -> Tuple[str, float]:
        """Detects language from a NumPy audio array."""
    ...
    def transcribe_chunks(
        ...
        """Transcribes a large audio array by slicing it into chunks in-memory."""
        all_segments = []
        full_text = []
        ...
        for idx in range(num_chunks):
            ...
            # Optimized: Passing NumPy array directly to model.transcribe avoids disk I/O
