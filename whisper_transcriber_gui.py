...
class AudioPreprocessor:
    ...
    def preprocess_to_memory(input_path: str, smart_silence: bool) -> np.ndarray:
        """
        Decodes audio to 16kHz mono float32 PCM data in-memory.
        Eliminates intermediate WAV files and redundant FFmpeg calls.
        """
        ...
        # Use a single pipe to avoid disk I/O and multiple subprocesses.
        # Redirect stderr to DEVNULL to avoid potential buffer deadlock.
        proc = subprocess.run(cmd, check=True, stdout=subprocess.PIPE, stderr=subprocess.DEVNULL)
        # Convert raw PCM S16LE bytes to normalized float32 numpy array.
...
class WhisperEngine:
    ...
    def detect_language(self, audio: np.ndarray) -> Tuple[str, float]:
        # whisper.pad_or_trim and log_mel_spectrogram work with numpy arrays
    ...
    def transcribe_chunks(
        ...
        for idx in range(total_chunks):
            ...
            # transcribe accepts numpy arrays directly, avoiding disk I/O
