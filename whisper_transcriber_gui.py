...
class AudioPreprocessor:
    ...
    def preprocess_to_memory(input_path: str, smart_silence: bool) -> np.ndarray:
        """
        Preprocesses audio directly into memory as a float32 NumPy array.
        ...
        # Pipe raw PCM data directly to stdout and read it into a NumPy array
...
class WhisperEngine:
    ...
    def detect_language(self, audio: np.ndarray) -> Tuple[str, float]:
        """
        Detects language from a NumPy array of audio samples.
        """
    ...
    def transcribe_chunks(
        ...
        """
        Transcribes multiple audio chunks in memory and merges the results.
        Correctly calculates offsets based on user-defined chunk size.
        """
        all_segments = []
        full_text = []
...
class TranscriberGUI:
    ...
    def _run_transcription(self) -> None:
        ...
        try:
            ...
            audio_full = AudioPreprocessor.preprocess_to_memory(input_file, self.silence_var.get())

            self._set_progress(0.12, "Detecting language")
            detected_lang, lang_prob = engine.detect_language(audio_full)
            ...
            self._log(f"Prepared {len(chunks)} chunk(s) via NumPy slicing")
